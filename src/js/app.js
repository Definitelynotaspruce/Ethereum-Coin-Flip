App = {
  ...
  initContract: function() {
    $.getJSON('CoinFlip.json', function(data) {
      ...
      // return App.markAdopted(); Show contract balance?
    }).then(function() {
  ...
  bindEvents: function() {
    ...
    $('#coin').on('click', function(){
      let betAmount = prompt("Enter how much you want to bet:", 10);
      App.handleBet(betAmount);
  ...
  handleBet: function(betAmount) {
    ...
    web3.eth.getAccounts(function(error, accounts) {
      ...
      App.contracts.CoinFlip.deployed().then(function(instance) {
        alert("Should run but..");
        betInstance = instance;
        alert(instance);
        // Execute adopt as a transaction by sending account
        return betInstance.bet().send({
          from: account,
          value: betAmount
        });
      }).then(function(result) {
